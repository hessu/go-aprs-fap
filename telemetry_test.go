...
func TestTelemetryClassic(t *testing.T) {
	...
	p, err := ParseAPRS("SRCCALL>APRS:T#324,000,038,255,.12,50.12,01000001")
	if err != nil {
...
func TestTelemetryRelaxed(t *testing.T) {
	...
	p, err := ParseAPRS("SRCCALL>APRS:T#1,-1,2147483647,-2147483648,0.000001,-0.0000001,01000001 comment")
	if err != nil {
...
func TestTelemetryShort(t *testing.T) {
	...
	p, err := ParseAPRS("SRCCALL>APRS:T#001,42")
	if err != nil {
...
func TestTelemetryUndefinedMiddle(t *testing.T) {
	...
	p, err := ParseAPRS("SRCCALL>APRS:T#1,1,,3,,5")
	if err != nil {
...
func TestTelemetryPartiallyCorrect(t *testing.T) {
	...
	p, err := ParseAPRS("SRCCALL>APRS:T#1,1,f,3")
	if err != nil {
...
func TestTelemetryInvalidDash(t *testing.T) {
	...
	_, err := ParseAPRS("SRCCALL>APRS:T#1,1,-,3")
	if err == nil {
		t.Fatal("expected error for invalid telemetry with bare dash")
	}
	p, _ := ParseAPRS("SRCCALL>APRS:T#1,1,-,3")
...
func TestTelemetryInvalidTrailingDot(t *testing.T) {
	...
	_, err := ParseAPRS("SRCCALL>APRS:T#1,1,-1.,3")
	if err == nil {
		t.Fatal("expected error for invalid telemetry with trailing dot")
	}
	p, _ := ParseAPRS("SRCCALL>APRS:T#1,1,-1.,3")
	if p.ResultCode != "tlm_inv" {
		t.Errorf("resultcode = %q, want %q", p.ResultCode, "tlm_inv")
	}
}
